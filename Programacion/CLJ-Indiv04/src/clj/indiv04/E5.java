...
"Enviar.";
        ...
        while(salir == false){
            ...
            System.out.println(
                "1. Buscar cuantas veces hay $Hyperboss\n"
                + "2. Buscar y reemplazar $Hyperboss por JEFAZO\n"
                + "3. Mostrar última frase en la que aparece $Hyperboss\n"
                + "0. Salir"
            );
            ...
            switch (opcion){
                case 1:
                    encontrado = (texto.length() - texto.replaceAll("\\$Hyperboss", "").length())/"$Hyperboss".length();
                    System.err.println(encontrado);
                    break;
                case 2:
                    texto = texto.replaceAll("\\$Hyperboss", ANSI_RED+"JEFAZO"+ANSI_RESET);
